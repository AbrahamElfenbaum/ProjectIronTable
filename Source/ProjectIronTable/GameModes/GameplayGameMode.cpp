...
void AGameplayGameMode::InitGame(const FString& MapName, const FString& Options, FString& ErrorMessage)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("AGameplayGameMode::InitGame — SessionInstance is null; session save will not be loaded"));
		return;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("AGameplayGameMode::InitGame — Failed to load session save for ID %s"), *SessionInstance->GetSessionID().ToString());
		return;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("AGameplayGameMode::InitGame — SessionGameState is null; cannot populate session data"));
		return;
	}
...
void AGameplayGameMode::PostLogin(APlayerController* NewPlayer)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("AGameplayGameMode::PostLogin — SessionPlayerState is null; cannot assign role flags"));
		return;
	}
	...
	if (!IsValid(SessionGameState))
	{
		UE_LOG(LogTemp, Warning, TEXT("AGameplayGameMode::PostLogin — SessionGameState is null; cannot determine role flags"));
		return;
	}
...
void AGameplayGameMode::Logout(AController* Exiting)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("AGameplayGameMode::Logout — ExitingPlayerController is null; cannot remove player from active list"));
		return;
	}
	...
	if (!IsValid(SessionPlayerState))
	{
		UE_LOG(LogTemp, Warning, TEXT("AGameplayGameMode::Logout — SessionPlayerState is null; cannot remove player from active list"));
		return;
	}
	...
	if (!IsValid(SessionGameState))
	{
		UE_LOG(LogTemp, Warning, TEXT("AGameplayGameMode::Logout — SessionGameState is null; cannot remove player from active list"));
		return;
	}
