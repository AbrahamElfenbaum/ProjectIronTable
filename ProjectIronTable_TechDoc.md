...
*Last updated: 2026-04-01* — Added `SaveLoad/` folder and `UPanelLayoutSave`/`FPanelLayoutData`. Updated `UDraggablePanel` with `PanelID`, `SetPanelID`, `GetPanelID`, `OnPanelStateChanged`, `GetPanelLayoutData`, `ApplyPanelLayoutData`. Updated `UTaskbarButton` with `OnToggled` delegate. Updated `UTaskbar::RegisterWidget` to return `UTaskbarButton*`. Updated `UGameplayHUDComponent` with `SavePanelLayout`, `LoadPanelLayout`, and updated `FindAndRegisterPanel`. Checked off panel layout persistence roadmap item. Updated Build.cs registered folders list.
