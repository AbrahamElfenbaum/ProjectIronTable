...
**Protected overrides:** `OnKeyChar` — calls `FindRunAtIndex`; if `FormatsMatch`, inserts directly into the found run; otherwise splits the run into Left/Middle/Right (Middle copies `ActiveFormat`, sets `.Text = FString(1, &Character)`), removes original, inserts three replacements; advances cursor; calls `SyncActiveFormat`. Guards `Character < 32`. `OnKeyDown` — detects `bShiftDown` (`IsLeftShiftDown() || IsRightShiftDown()`); pre-computes `Lines` (from `GetFullText()`), `CursorPos`, `TabSpace`; handles Backspace/Delete via `OnBackspaceOrDeletePressed`; Left/Right call `HandleSelectionOnMove(bShiftDown)` before updating `CursorPosition`, then call `SyncActiveFormat`; Up/Down call `HandleSelectionOnMove(bShiftDown)` before `OnUpOrDownPressed`; Enter/Tab → `DrawSpecialCharacter`; Ctrl+B/I/U toggle `ActiveFormat` flags and call `SetIsChecked`. `OnMouseButtonDown` — calls `SetKeyboardFocus`; resets `SelectionAnchor = -1`; sets `CursorPosition = GetAreaCursorPosition(MouseEvent)`. `OnMouseMove` — if left button held, updates `CursorPosition = GetAreaCursorPosition(MouseEvent)` (anchor stays fixed); returns `Handled` only while button is held.
...
| 2.9   | Column drift on repeated Up/Down — each press recomputes `CursorPos.X` from the current `CursorPosition`; landing on a slightly different X shifts the target for the next press and compounds over multiple presses | `RichTextEditor.cpp`   | Known |
...
- [~] Shared notes — `SRichTextEditor` + `SRichTextArea` functional: multi-run document model, format-aware insertion, cursor sync, Ctrl+B/I/U, multi-run backspace/delete, run merging, visual per-run rendering, and selection rendering all working. Selection model: fixed `SelectionAnchor` + moving `CursorPosition`; `GetSelectionMin/Max()` inline helpers; `GetSelectedRange()` returns clipped run array; `HandleSelectionOnMove(bShiftDown)` wires Shift+Arrow; `DrawHighlight` draws per-line selection rects in `SRichTextArea::OnPaint`. Known bug 2.6 (consecutive tabs). Known bug 2.9 (column drift on Up/Down). Pending: drag select, column drift fix (`PreferredX`), selection-aware editing (delete/type replaces selection), Ctrl+A, Ctrl+C/X, format-to-selection, `USessionNotesPanel` override bodies, save/load, input context (`IMC_Notes`)
...
*Last updated: 2026-04-30* — Dead code block removed from `GetSelectedRange`. Changelogs removed from TechDoc and GDD — full history lives in `ProjectIronTable_Changelog.md`; both docs now hold only a single `*Last updated*` one-liner. `/update-docs` command updated to include changelog step.
