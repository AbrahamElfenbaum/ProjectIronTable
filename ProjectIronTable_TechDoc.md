...
*Last updated: 2026-03-29* — Added `UTaskbarButton` and `UTaskbar` classes. Updated `UGameplayHUDComponent` to document `TaskbarRef` and `RegisterWidget` calls for Chat, Dice, and Players. Added taskbar minimize sub-item to Phase 6 roadmap. Added two new gotchas: taskbar toggle visibility check and Blueprint widget variant placement.
