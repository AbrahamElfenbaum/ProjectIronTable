...
*Last updated: 2026-05-09* — Toolbar moved from `SRichTextEditor` to `USessionNotesChannel` (`BindWidget` UCheckBoxes + `OnFormatStateChanged` delegate); `FillHeight` collapse fixed and text box now fills panel correctly; `OnKeyDown` `bDocumentModified` flag added (fixes scroll-on-Enter); `OnUpOrDownPressed` extended with `TabSpace` for per-character walk (fixes bug 2.6).
