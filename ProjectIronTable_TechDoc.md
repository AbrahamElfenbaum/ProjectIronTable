...
**Static helper — `HitTest(FVector2f LocalMousePos, const FRichTextDocument& InDocument, float InScale) → int32`:** Inverse of `GetCursorPosition`. Computes `LineHeight` via `GetMaxCharacterHeight`; parses full text into `Lines`; clamps `TargetLine = FMath::Clamp((int32)(LocalMousePos.Y / LineHeight), 0, Lines.Num() - 1)`; walks characters in `Lines[TargetLine]` measuring cumulative widths until exceeding `LocalMousePos.X`; accumulates document index by summing preceding line lengths + 1 per newline, then adds `CharInLine`. Called by `SRichTextEditor::OnMouseButtonDown` and `OnMouseMove`.
...
- `FormatToSelection(TFunction<void(FRichTextRun&)> ApplyFormat)` — if `SelectionAnchor == -1`, no-ops. Otherwise: splits the run at `SelectionMin` (RemoveAt → Insert Left → Insert Right); splits the run at `SelectionMax` via a second `FindRunAtIndex` call (indices shift by 1 due to first split); walks runs from `MinRunIndex + 1` to `MaxRunIndex` inclusive, calling `ApplyFormat` on each; calls `PruneRuns()`. Called by all four toggle functions.
