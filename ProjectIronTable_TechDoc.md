...
*Last updated: 2026-04-01* — Home screen implemented: `AHomeScreenController`, `UHomeScreenHUDComponent`, `GM_HomeScreen`, `PC_HomeScreen`, `S_HomeScreen`, `L_HomeScreen`. Scene/controller management complete. Level naming convention added (L_ = final, Dev_ = dev/test). `UGameplayHUDComponent` input mode and cursor now owned by `AGameplayController`.
