...
- `OnUpOrDownPressed(Lines, CursorPos, Scale, bUp)` — if `PreferredX < 0`, captures `PreferredX = CursorPos.X`. Bounds-checks target line, walks characters using midpoint comparison: caches `LeftEdge` per iteration, computes `Midpoint = (LeftEdge + RightEdge) / 2`; breaks before updating `CharInLine` when `Midpoint > PreferredX`. Converts to document index. Midpoint comparison prevents floating-point off-by-one that caused the cursor to consistently land one character short of the original column.
