...
- **Client** (`SaveToClient`/`LoadFromClient`) — per-account → EOS Player Data Storage via `IOnlineUserCloud`, keyed to the ProductUserId. Thin one-line wrappers over the shared `WriteCloud`/`ReadCloud` helpers. `HandleReadUserFileComplete` creates a fresh save on **not-found** but fails on no-data/corrupt.
- **Server** (`SaveToServer`/`LoadFromServer`) — authority-owned shared state, **listen-host, DONE & round-trip verified**. A net-mode authority gate (a subsystem has no `HasAuthority()`, so `GetGameInstance()->GetWorld()->GetNetMode() == NM_Client` → reject; Standalone/ListenServer/DedicatedServer proceed) then routes to the same `WriteCloud`/`ReadCloud` as the client bucket, persisting to the **host's** PDS (`GetUniquePlayerId(0)` on a listen host = the host). The gate keeps a future dedicated-server switch seamless.

Four private helpers absorb the repeated online-subsystem boilerplate. `GetUserCloud` (resolves OS + user cloud interface) and `GetCloudAndUser` (reuses it, adds identity + `GetUniquePlayerId(0)`) **resolve without firing** — they return `false` and set an `OutError` string so the *caller* fires the correct delegate (save 2-param vs load 3-param, so a shared fire is impossible); `Context` is the caller's function name, logged via `%s`. `WriteCloud`/`ReadCloud` (both `void`, non-const) own the full serialize→register→stash→write (and register→stash→read) path, giving client and server a single backend seam.
...
**`FVisualLine` struct (in `RichTextArea.h`):** Character index range for one rendered line.

| Field        | Type    | Notes                                         |
|--------------|---------|-----------------------------------------------|
| `StartIndex` | `int32` | Index of the first character on this line     |
| `EndIndex`   | `int32` | Exclusive — index one past the last character |

**Private state:**

| Field             | Type                          | Notes                                                              |
|-------------------|-------------------------------|--------------------------------------------------------------------|
| `Document`        | `const FRichTextDocument*`    | Not owned — pointer into `SRichTextEditor`                         |
| `CursorPosition`  | `const int32*`                | Pointer into `SRichTextEditor` — always in sync                    |
| `SelectionAnchor` | `const int32*`                | -1 when no selection is active                                     |
| `CachedWidth`     | `mutable float`               | Last allotted width; -1 until first paint; triggers rebuild on change |
| `CachedText`      | `mutable FString`             | Snapshot of document text at last rebuild; triggers rebuild on change |
| `VisualLines`     | `mutable TArray<FVisualLine>` | Pre-computed char index ranges per visual line; rebuilt by `RebuildVisualLines` |
...
| Field             | Type                        | Notes                                                                |
|-------------------|-----------------------------|----------------------------------------------------------------------|
| `Document`        | `FRichTextDocument`         | The full document model                                              |
| `CursorPosition`  | `int32` (default 0)         | Character index of the cursor                                        |
| `SelectionAnchor` | `int32` (default -1)        | Fixed anchor point; -1 means no selection                            |
| `PreferredX`      | `float` (default -1)        | Target X held across Up/Down presses; reset on non-vertical key/click |
| `ActiveFormat`    | `FRichTextRun`              | Format carrier for newly typed text; `Text` field unused             |
...
- **Online-subsystem shared-ptr types (`IOnlineUserCloudPtr`, `FUniqueNetIdPtr`) are typedefs — they cannot be forward-declared.** Forward-declaring the underlying class (`IOnlineUserCloud`) doesn't make the typedef name visible. Either spell out the `TSharedPtr<..., ESPMode::ThreadSafe>` in the signature (forward-declaring the class), or include the interface headers (`Interfaces/OnlineUserCloudInterface.h`, `Online/CoreOnline.h`) in the `.h`. Also: `FUniqueNetId` is abstract, so an out-param must be the ptr (`FUniqueNetIdPtr`), never a bare `FUniqueNetId&`.
- **`UGameInstanceSubsystem` is NOT in the console `Exec` routing chain** — a `UFUNCTION(Exec)` on a subsystem is unreachable from the `~` console. Console commands route through PlayerController → Pawn → GameMode → GameInstance, so a dev/exec command must live on one of those (e.g. the GameInstance) and forward into the subsystem.
- **UE Live Coding can leave a stale/mixed binary in the running editor** — a tell is one PIE run logging *both* an old and a new version of the same string (e.g. a renamed log). Live Coding patches some code paths but not already-instantiated ones. Per the build workflow: do a full Visual Studio build with the **editor closed**; if it persists, delete `Intermediate/`+`Binaries/`, regenerate project files, and rebuild.
...
*Last updated: 2026-07-03* — `USaveLoadSubsystem` server bucket **DONE & round-trip verified** (listen-host: net-mode authority gate — `GetGameInstance()->GetWorld()->GetNetMode() == NM_Client` rejects, authority modes proceed — then the shared cloud path to the host's PDS). Extracted the repeated online-resolve boilerplate into four private helpers: `GetUserCloud`/`GetCloudAndUser` (resolve-without-firing: return `false` + set `OutError`, caller fires the right delegate) and `WriteCloud`/`ReadCloud` (the full serialize→register→stash→write/read path); `SaveToClient`/`LoadFromClient` collapsed to one-line wrappers, `SaveToServer`/`LoadFromServer` = gate + wrapper. Added `Engine/GameInstance.h` + `Engine/World.h` includes. New gotchas recorded: online-subsystem typedefs can't be forward-declared, subsystems aren't in the console `Exec` chain, and UE Live Coding can leave a stale/mixed binary.
